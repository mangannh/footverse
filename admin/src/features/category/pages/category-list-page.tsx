...
/**
 * The category list screen — the React analog of the Flutter
 * `AddressListScreen` (sprint-10-plan Task 05). Mirrors `BrandListPage`
 * (Task 04) verbatim for the category domain. It owns [useCategoryList] (the
 * load state machine) and [useCategoryMutation] (delete), wiring the
 * mutation's reload to the list's own `load` so a successful delete always
 * re-reads the server-owned list. Create / edit are routed to
 * [ROUTES.categoryForm]; that page's own success reload (navigating back here)
 * re-mounts this page, which re-fetches on mount.
 */
export function CategoryListPage({ repository }: CategoryListPageProps): ReactElement {
  const list = useCategoryList(repository);
  const { load } = list;
  const mutation = useCategoryMutation(repository, load);
    ...
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        ...
        <Button
          ...
          onClick={() => navigate(ROUTES.categoryForm)}
        >
          New category
        </Button>
      </Stack>
      ...
      {list.status === 'ready' && list.categories.length === 0 && (
      ...
      {list.status === 'ready' && list.categories.length > 0 && (
        <CategoryTable
          ...
          disabled={mutation.isMutating}
          onEdit={(category) => navigate(ROUTES.categoryForm, { state: { category } })}
